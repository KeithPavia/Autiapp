...
export function HomePage() {
  ...
  const setCelebrationMode = (mode: 'full' | 'calm' | 'silent') => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-purple-50 px-6 pt-28 pb-24">
      <div className="max-w-md mx-auto">
        ...
        <div className="fixed top-6 right-6 z-50">
          <button
            onMouseDown={startParentHold}
            onMouseUp={stopParentHold}
            onMouseLeave={stopParentHold}
            onTouchStart={startParentHold}
            onTouchEnd={stopParentHold}
            onTouchCancel={stopParentHold}
            className={`w-14 h-14 rounded-full shadow-lg transition-all flex items-center justify-center ${
              holdActive
                ? 'bg-gradient-to-br from-amber-500 to-orange-500 scale-105'
      ...
      {showParentSettings && (
        <div className="fixed inset-0 z-[80] bg-black/45 flex items-center justify-center px-6">
          <div className="w-full max-w-md rounded-[2rem] bg-white p-7 shadow-2xl">
            ...
            <div className="space-y-4">
              <button
                ...
                className="w-full rounded-[1.5rem] bg-gradient-to-r from-green-400 to-emerald-500 text-white text-left p-5 shadow-lg active:scale-95"
              >
                <div className="text-2xl font-bold">🎉 Full</div>
                <div className="text-base opacity-95">Confetti + gentle “Good job” + applause</div>
              </button>

              <button
                onClick={() => setCelebrationMode('calm')}
                className="w-full rounded-[1.5rem] bg-gradient-to-r from-blue-400 to-cyan-500 text-white text-left p-5 shadow-lg active:scale-95"
              >
                <div className="text-2xl font-bold">😊 Calm</div>
                <div className="text-base opacity-95">Less confetti + gentle “Good job” only</div>
              </button>

              <button
                onClick={() => setCelebrationMode('silent')}
                className="w-full rounded-[1.5rem] bg-gradient-to-r from-slate-400 to-slate-500 text-white text-left p-5 shadow-lg active:scale-95"
              >
                <div className="text-2xl font-bold">🔇 Silent</div>
                <div className="text-base opacity-95">Visual celebration only</div>
              </button>
            </div>

            <button
              onClick={() => setShowParentSettings(false)}
              className="mt-5 w-full rounded-[1.5rem] bg-white border border-slate-200 text-slate-700 text-2xl py-4 shadow active:scale-95"
            >
              Close
            </button>
          </div>
